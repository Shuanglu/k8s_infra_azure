...
while [ "$continue" == 0 ]
do
  read -p "Please input the token: " token
...
if [ "$nodetype" == "Master" ]; then
  az vmss run-command invoke --command-id RunShellScript --scripts 'cd /var/log/; if [ -f scripts.tar ]; then rm -rf scripts*;fi;timeout 30 wget https://$1.blob.core.windows.net/$2/scripts.tar --quiet; if [ $? -ne 0 ]; then echo "failed to download" && exit 1;fi; tar -xvf scripts.tar > /dev/null 2>&1 && cd scripts && chmod -R 777 * && bash -c "./main.sh $3 $4" >>/var/log/scripts/initilization.log 2>&1; if [ $? -eq 0 ];then echo "succeed";else echo "failed";fi' --parameters $sa $container $token $apiserver -g k8s-infra -n k8s-master --instance-id 0
fi
